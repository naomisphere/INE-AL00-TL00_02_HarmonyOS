...
int symbol_logical(const char *for_check, size_t i)
{
  if ((*(for_check + i) == ':') &&
      (*(for_check + i + 3) == ':') &&
      (*(for_check + i + 6) == ':') &&
...
int char_hexadecimal(const char *for_check)
{
  if (((*for_check >= '0') && (*for_check <= '9')) ||
...
void change_ip(char *ip_input)
{
  char *temp = ip_input;
  int dot_num = 0;

  /* In order to mask the last 1 byte, just like
     192.168.100.100 --> 192.168.100.***, we need
     replace the digital with '*' after the 3th dot. */
  while (*temp != '\0') {
    if (dot_num == 3) {
      *temp++ = '*';
      continue;
...
void change_mac(char *mac_input)
{
  if (mac_input == NULL) {
    return ;
  }
  ...
  /* The normal size of MAC adress xx:xx:xx:xx:xx:xx 17 */
  if (len < 17) {
    return ;
  }

  /* start to traverse strings, find MAC adress */
  size_t i = 2;

  /* the variable 'i' is where possible the first ':' location, 'i+14' is the end of MAC address , */
  /* only the end of MAC address is not '\0'  indicated the input have the storage space */
  while (*(mac_input + i + 14) != '\0') {
    int mac_address = 1;
    /* firstly, check whether there is a ":" between every two characters , and there are five ':' */
    if (symbol_logical(mac_input, i)) {
      /* secondly, check whether other characters are hexadecimal */
      int j = -2;
      while (j != 15) {
        if (j % 3 == 0) {
          ++j;
        ...
        if (char_hexadecimal(mac_input + i + j)) {
          ++j;
          continue;
        } else {
          ++j;
    ...
    if (!mac_address) {
      /* If it is not MAC address ,go to the next loop */
      ++i;
    } else {
      /* If it is MAC address, modify 4 5,7 8,10 11, to '*' */
      int m = 1;
      while (m != 9) {
        if (m % 3 == 0) {
          ++m;
      ...
      /* after the MAC address, do not have enough storage space for next MAC address,break */
      if (i + 17 + 15 > len) {
        break;
      } else {
        /* have enouth storage, jump to the end for the MAC address */
        i += 17;
      }
    }
  }
		       ...
		       int mac_len, char *interface, char *string, u32 xid)
{
  struct in_addr a;
  int ret;
  static char mac_buf[OPTION_END] = {0};
  static char ip_buf[OPTION_END] = {0};
  ...
  ret = sprintf_s(mac_buf, OPTION_END, "%s", daemon->namebuff);
  if (ret < 0) {
    printf("mac sprintf_s fail\n");
    return;
  }
  change_mac(mac_buf);

  ret = sprintf_s(ip_buf, OPTION_END, "%s", inet_ntoa(a));
  if (ret < 0) {
    printf("ip sprintf_s fail\n");
    return;
  }
  change_ip(ip_buf);
