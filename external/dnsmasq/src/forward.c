...
static struct frec *lookup_frec_by_sender(unsigned short id,
		       ...
		       void* hash);
static struct frec* lookup_frec_by_query(void* hash);
			 ...
			 HEADER *header, size_t plen, time_t now, struct frec *forward)
{
  ...
  if (!daemon->servers)
    forward = NULL;
  else if (forward || (forward = lookup_frec_by_sender(ntohs(header->id), udpaddr, hash)))
    {
  ...
  else 
    {
      ...
      if ((forward = lookup_frec_by_query(hash))) {
        /* Note whine_malloc() zeros memory. */
        if (!daemon->free_frec_src && daemon->frec_src_count < daemon->ftabsize &&
...
void reply_query(int fd, int family, time_t now)
{
  ...
  if (!server ||
      ...
      !(forward = lookup_frec(ntohs(header->id), fd, family, hash)))
  ...
  if (forward->forwardall == 0 || --forward->forwardall == 1 || 
      ...
      if ((nn = process_reply(header, now, server, (size_t)n)))
	{
	  ...
	  send_from(forward->fd, daemon->options & OPT_NOWILD, daemon->packet, nn, 
			&forward->frec_src.source, &forward->frec_src.dest, forward->frec_src.iface);
	}
      free_frec(forward); /* cancel */
    }
}
...
static struct frec *lookup_frec(unsigned short id, int fd, int family, void* hash)
{
  ...
  for(f = daemon->frec_list; f; f = f->next)
    if (f->sentto && f->new_id == id && (memcmp(hash, f->hash, HASH_SIZE) == 0))  {
...
{
  struct frec* f = NULL;
  struct frec_src* src = NULL;
  for (f = daemon->frec_list; f; f = f->next)
      if (f->sentto && memcmp(hash, f->hash, HASH_SIZE) == 0)
          for (src = &f->frec_src; src; src = src->next)
              if (src->orig_id == id && sockaddr_isequal(&src->source, addr))
                  return f;
  return NULL;
}
static struct frec* lookup_frec_by_query(void* hash)
