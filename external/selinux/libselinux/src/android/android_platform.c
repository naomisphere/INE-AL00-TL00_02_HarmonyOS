...
#ifdef BG_FSCK
#define FS_MGR_USERDATA_PARTITION (char*)"/dev/block/bootdevice/by-name/userdata"
#endif

// For 'system', 'vendor' (mandatory) and/or 'odm' (optional).
#define MAX_FILE_CONTEXT_SIZE 3

enum seapp_debug_log {
    SEAPP_CONTEXT_SYSTEM_EXIST  = 1<<0,
    SEAPP_CONTEXT_SYSTEM_NOT_EXIST  = 1<<1,
    SEAPP_CONTEXT_VENDOR_EXIST  = 1<<2,
    SEAPP_CONTEXT_VENDOR_NOT_EXIST  = 1<<3,
    SEAPP_CONTEXT_ALLOC_MEMORY_FILE_OPEN_ERROR = 1<<4,
    SEAPP_CONTEXT_GET_LABEL_FILE_OPEN_ERROR  = 1<<5
};
static unsigned int seapp_log_flag = 0;
...
int selinux_android_seapp_context_reload(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(seapp_contexts_plat); i++) {
		if (access(seapp_contexts_plat[i], R_OK) != -1) {
			seapp_log_flag |= SEAPP_CONTEXT_SYSTEM_EXIST;
			seapp_contexts_files[files_len++] = seapp_contexts_plat[i];
			break;
		} else {
			seapp_log_flag |= SEAPP_CONTEXT_SYSTEM_NOT_EXIST;
			selinux_log(SELINUX_ERROR, "%s: seapp_contexts file: %s not exist",
				    __FUNCTION__, seapp_contexts_plat[i]);
		}
	}
	for (i = 0; i < ARRAY_SIZE(seapp_contexts_vendor); i++) {
		if (access(seapp_contexts_vendor[i], R_OK) != -1) {
			seapp_log_flag |= SEAPP_CONTEXT_VENDOR_EXIST;
			seapp_contexts_files[files_len++] = seapp_contexts_vendor[i];
			break;
		} else {
			seapp_log_flag |= SEAPP_CONTEXT_VENDOR_NOT_EXIST;
			selinux_log(SELINUX_ERROR, "%s: seapp_contexts file: %s not exist",
				    __FUNCTION__, seapp_contexts_vendor[i]);
		}
	}
	...
	for (i = 0; i < files_len; i++) {
		...
		if (!fp) {
			seapp_log_flag |= SEAPP_CONTEXT_ALLOC_MEMORY_FILE_OPEN_ERROR;
	...
	for (i = 0; i < files_len; i++) {
		...
		fp = fopen(seapp_contexts_files[i], "re");
		if (!fp) {
			seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_OPEN_ERROR;
			selinux_log(SELINUX_ERROR, "%s:  could not open seapp_contexts file: %s",
				    __FUNCTION__, seapp_contexts_files[i]);
			free_seapp_contexts();
			return -1;
...
#warning "This code assumes that getpwuid is thread safe, only true with Bionic!"
	...
	if (kind == SEAPP_DOMAIN && i == nspec) {
		 ...
		 */
		selinux_log(SELINUX_ERROR,
			    "%s:  No match for app with uid %d, seinfo %s, name %s, nspec %d,seapp_log_flag %#x\n",
			    __FUNCTION__, uid, seinfo, pkgname, nspec, seapp_log_flag);
...
#define DATA_MEDIA_0_PATH "/data/media/0"
...
err:
    selinux_log(SELINUX_ERROR, "%s:  Error looking up context for path %s, pkgname %s, seinfo %s, uid %u: %s\n",
                __FUNCTION__, pathname, pkgname, (info ? info->seinfo : seinfo), (info ? info->uid : uid), strerror(errno));
    rc = -1;
    goto out;
}
...
#ifdef BG_FSCK
static int restorecon_sb_userdata(const char *pathname, const struct stat *sb,
                         bool nochange, bool verbose)
{
    char *secontext = NULL;
    char *oldsecontext = NULL;
    ...
    const char* links[2] = { FS_MGR_USERDATA_PARTITION, NULL };
    if (selabel_lookup_best_match(fc_sehandle, &secontext, pathname, links, sb->st_mode) < 0){
        selinux_log(SELINUX_INFO,
                "SELinux: not found secontext for %s !\n" , pathname);
        return 0;  /* no match, but not an error */
    }
                                             ...
                                             unsigned int flags)
{
...
#ifdef BG_FSCK
    char data_partition[32] = { 0 };
    if(!strcmp(pathname, "/dev/block")){
        int ret = readlink(FS_MGR_USERDATA_PARTITION, data_partition, sizeof(data_partition));
        if (ret < 0){
            selinux_log(SELINUX_ERROR, "%s : readlink for %s failed!\n", __func__, FS_MGR_USERDATA_PARTITION);
            memset(data_partition, 0, sizeof(data_partition));
        }
    }
#endif
    ...
    while ((ftsent = fts_read(fts)) != NULL) {
        ...
        case FTS_D:
            ...
            if (!datadata &&
                (!strncmp(ftsent->fts_path, DATA_MEDIA_0_PATH, sizeof(DATA_MEDIA_0_PATH)-1))) {
                    selinux_log(SELINUX_ERROR,
                        "SELinux:  Skip restorecon %s: %s.\n", ftsent->fts_path, strerror(errno));
                    fts_set(fts, ftsent, FTS_SKIP);
            }
        ...
        default:
#ifdef BG_FSCK
            if(!strcmp(data_partition, ftsent->fts_path)){
                error |= restorecon_sb_userdata(ftsent->fts_path, ftsent->fts_statp, nochange, verbose);
                break;
            }
#endif
...
bool hw_compute_file_contexts_hash(uint8_t fc_digest[])
{
    struct selinux_opt seopts_file[MAX_FILE_CONTEXT_SIZE];
    int size = 0;
    unsigned int i;
    ...
    return compute_file_contexts_hash(fc_digest, seopts_file, size);
}
